import { assertHerdr, herdrJson, herdrText } from "./herdr.js";
import type {
  AgentMetadata,
  AgentRuntime,
...
function dimensions(value: unknown): { width: number; height: number } | undefined {
  ...
  if (width !== undefined && height !== undefined && width > 0 && height > 0) return { width, height };
...
function findPaneDimensions(value: unknown, paneId: string): { width: number; height: number } | undefined {
...
export class HerdrRuntime implements AgentRuntime {
  ...
  async createLocation(options: CreateLocationOptions): Promise<RuntimeLocation> {
    if (options.worktree) {
      if (!options.branch) throw new Error("Herdr worktree runtime requires a branch name.");
      ...
      if (!rootPaneId) throw new Error("Herdr worktree create did not return result.root_pane.pane_id.");
      ...
      if (!paneId) throw new Error("Herdr worktree agent pane split did not return result.pane.pane_id.");
      await herdrJson(["pane", "close", rootPaneId]);
      return {
        paneId,
        workspaceId: created.result?.workspace?.workspace_id ?? created.result?.workspace?.id,
        cwd,
      };
    }

    const callerPane = process.env.HERDR_PANE_ID;
    if (!callerPane) throw new Error("HERDR_PANE_ID is required to create a sibling agent pane.");
    const direction = options.direction ?? (await this.chooseSplitDirection(callerPane));
    const split = await herdrJson<PaneSplitResult>([
      "pane",
      "split",
      callerPane,
    ...
    const paneId = split.result?.pane?.pane_id;
    if (!paneId) throw new Error("Herdr pane split did not return result.pane.pane_id.");
    return { paneId, cwd: options.cwd };
  }

  async start(name: string, paneId: string, agentArgs: string[]): Promise<RuntimeAgentState> {
  ...
  async prompt(name: string, text: string): Promise<RuntimeAgentState> {
    const result = await herdrJson<AgentResult>(["agent", "prompt", name, text]);
    return statusFromResult(result.result);
  }
  ...
  async read(name: string, lines: number): Promise<string> {
    try {
      return await herdrText(["agent", "read", name, "--source", "recent-unwrapped", "--lines", String(lines)]);
    } catch {
      return herdrText(["agent", "read", name, "--source", "visible", "--lines", String(lines)]);
    }
  }
  ...
  private async chooseSplitDirection(callerPane: string): Promise<"right" | "down"> {
    try {
      ...
      const response = await herdrJson<LayoutResult>(["pane", "layout", "--pane", callerPane]);
      const dims = findPaneDimensions(response.result, callerPane) ?? dimensions(response.result?.area);
